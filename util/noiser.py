...
class DSBNoiser(BaseNoiser):
    def __init__(self, *args, mean=0, std=1, **kwargs):
        ...
        self.prepare_gamma_dsb()
        self.gamma_cumsum = torch.cumsum(torch.cat([torch.zeros(size=(1,), dtype=torch.float32, device=self.device), self.gammas], dim=0), dim=0)
        self.gamma_cumsum = self.gamma_cumsum / self.gamma_cumsum[-1]
