        ...
        if dsb:
            if self.cnt + self.args.batch_size > self.cache_size:
                ...
                for _ in pbar:
                    ...
                    with torch.no_grad():
                        if self.direction == 'forward' and epoch == 0:
                            ...
                            model.eval()
                            _x_cache, _gt_cache, _t_cache = model.inference(x_1 if self.direction == 'forward' else x_0)
    ...
    def train(self):
        ...
        for epoch in range(self.args.epochs):
            ...
            if self.dsb:
                ...
                self.direction = 'forward' if epoch % 2 == 0 else 'backward'
    ...
    def evaluate(self, epoch, iters, last=False):
        with torch.no_grad():
            ...
            if self.args.global_rank == 0:
                ...
                if self.dsb:
                    ps = self.forward_model.inference(x_prior, sample=True)[0]
                    if epoch == 0:
                        qs = self.noiser.trajectory_dsb(x_prior, x_data, sample=True)[0]
                    else:
                        qs = self.backward_model.inference(x_data, sample=True)[0]
                else:
                    ps = self.model.inference(x_prior, return_all=True)[1]
                    qs = self.noiser.trajectory(x_prior, x_data)

                x_prior, x_data, ps, qs = x_prior.to(self.device), x_data.to(self.device), ps.to(self.device), qs.to(self.device)
                ...
                if self.dsb:
                    ...
                    self.evaluators['Inference'].draw(epoch, iters, ps[-1])
